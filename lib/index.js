...
export default class Jasql {
  ...
  initialize () {
    return this.db.schema
      ...
      .then((exists) => {
        if (!exists) {
          return this.db.schema.createTable(this.tableName, this._tableSchema)
        }
      })
  }
  ...
  _tableSchema (table) {
    ...
    table.string(this.idColName, 255).index().unique()
    table.specificType(this.jsonColName, this.jsonColType)
  }
}
